...
impl<T> PolyContext<T>
where
    T: Ntt,
{
    // Creates a new polynomial context
    pub fn new(moduli: &[u64], degree: usize) -> PolyContext<T> {
        let moduli_ops = moduli
            .iter()
            .map(|modulus| Modulus::new(*modulus))
            .collect_vec();

        let ntt_ops = moduli.iter().map(|m| T::new(degree, *m)).collect_vec();
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Poly<T: Ntt> {
    pub coefficients: Array2<u64>,
    pub representation: Representation,
    pub context: Arc<PolyContext<T>>,
}
    ...
    T: Ntt,
{
    pub fn new(
        ...
        poly_context: &Arc<PolyContext<T>>,
    ...
    pub fn zero(poly_context: &Arc<PolyContext<T>>, representation: &Representation) -> Poly<T> {
        Poly {
    ...
    pub fn scale_and_round(
        &self,
        out_context: &Arc<PolyContext<T>>,
        p_context: &Arc<PolyContext<T>>,
        q_context: &Arc<PolyContext<T>>,
    ...
    pub fn fma_reverse_inplace(&mut self, p1: &Poly<T>, p2: &Poly<T>) {
    ...
    pub fn fma_reverse(&self, p1: &Poly<T>, p2: &Poly<T>) -> Poly<T> {
...
impl<T> MulAssign<&Poly<T>> for Poly<T>
where
    T: Ntt,
{
...
impl<T> From<&Poly<T>> for Vec<BigUint>
where
    T: Ntt,
{
    fn from(p: &Poly<T>) -> Vec<BigUint> {
