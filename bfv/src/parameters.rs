...
pub struct BfvParameters<T: Ntt> {
    ...
    pub ciphertext_poly_contexts: Box<[Arc<PolyContext<T>>]>,
    pub extension_poly_contexts: Box<[Arc<PolyContext<T>>]>,
    pub pq_poly_contexts: Box<[Arc<PolyContext<T>>]>,
    ...
    // Encryption
    pub ql_modt: Vec<u64>,
    pub neg_t_inv_modql: Vec<Poly<T>>,
