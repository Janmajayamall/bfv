    ...
    T: Ntt,
{
    ...
    pub fn new(coefficients: Vec<i64>, params: &Arc<BfvParameters<T>>) -> SecretKey<T> {
    ...
    fn to_poly(&self, level: usize) -> Poly<T> {
    ...
    pub fn encrypt<R: CryptoRng + RngCore>(&self, pt: &Plaintext<T>, rng: &mut R) -> Ciphertext<T> {
    ...
    pub fn decrypt(&self, ct: &Ciphertext<T>) -> Plaintext<T> {
    ...
    pub fn measure_noise<R: CryptoRng + RngCore>(&self, ct: &Ciphertext<T>, rng: &mut R) -> u64 {
